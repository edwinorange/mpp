...
- **Type:** `Readonly<Record<string, Attestation.Signer>>`

Request-attestation signers applied to the initial HTTP request and every automatic payment retry. Each attempt uses a fresh shared nonce and timestamp. Configure [Web Bot Auth or Trusted Agent Protocol](/advanced/identity#request-attestation), or include both signers when a request needs both signatures.

:::code-group

```ts twoslash [Web Bot Auth]
...
const client = Mppx.create({
  ...
  attestation: {
    webBotAuth: WebBotAuth.Client.signer({
      key: botPrivateKey,
      keyId: process.env.WEB_BOT_AUTH_KEY_ID!,
      signatureAgent: process.env.WEB_BOT_AUTH_DIRECTORY!,
    }),
  },
  // [!code hl:end]
  methods: [tempo({ account })],
...
```ts twoslash [Trusted Agent Protocol]
